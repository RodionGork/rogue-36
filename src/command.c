...
command()
{
	...
	else
	{
		...
		otherwise :
			...
			when 'C' : create_obj();
			when CTRLBTN('I') : inventory(lvl_obj, 0);
			when CTRLBTN('W') : whatis();
			when CTRLBTN('D') : level++; new_level();
			when CTRLBTN('U') : level--; new_level();
			when CTRLBTN('F') : show_win(stdscr, "--More (level map)--");
			when CTRLBTN('X') : show_win(mw, "--More (monsters)--");
			when CTRLBTN('T') : teleport();
			when CTRLBTN('E') : msg("food left: %d", food_left);
			when CTRLBTN('A') : msg("%d things in your pack", inpack);
			when CTRLBTN('C') : add_pass();
			when CTRLBTN('N') :
			{
